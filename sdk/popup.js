...
function createPopup() {
    ...
    popupButton.onclick = () => trackClickAndNavigate();
...
function showPopup(imageUrl, locationUrl) {
    console.log("=== Show Pic: ", imageUrl);
    document.getElementById('popupImage').src = imageUrl;
    document.getElementById('popupButton').dataset.location = locationUrl; // Store location in data attribute
...
// Function to track button click and navigate
function trackClickAndNavigate() {
    const locationUrl = document.getElementById('popupButton').dataset.location;
    ...
    .then(data => {
        ...
        hidePopup();
        if (locationUrl) {
            window.location.href = locationUrl; // Redirect to the location
        }
    })
        ...
        .then(data => {
            ...
            if parseInt(data.value) != -1 {
                if (data.url && data.location) {
                    showPopup(data.url, data.location);
                }
            };
        })
